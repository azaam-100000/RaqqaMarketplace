  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        ...
        const { data: groupsData, error: groupsError } = await query.order('created_at', { ascending: false });
        ...
        const groupsWithProfiles = groupsData.map((g) => ({
          ...g,
          profiles: profilesMap.get(g.user_id) || null,
        }));

        setGroups(groupsWithProfiles as Group[]);
