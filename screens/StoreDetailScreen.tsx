...
const StoreDetailScreen: React.FC = () => {
    ...
    const handleDeleteStore = async () => {
        if (!store || !window.confirm(`هل أنت متأكد من حذف متجر "${store.name}"؟`)) return;
        try {
            if (store.image_url) await supabase.storage.from('uploads').remove([store.image_url]);
            const { error } = await supabase.from('stores').delete().eq('id', store.id);
            if (error) throw error;
            navigate('/stores');
        } catch (err: unknown) { alert(`فشل حذف المتجر: ${getErrorMessage(err)}`); }
    };

    const renderProducts = () => {
        if (products.length === 0) return <div className="text-center text-slate-400 py-10"><p>لا توجد منتجات في هذا المتجر حتى الآن.</p></div>;
        ...
        <div className="min-h-screen bg-slate-900 text-white">
                ...
                <div className="container mx-auto px-4">
                     <div className="flex items-center h-16 relative">
                        ...
                        <h1 className="text-xl font-bold text-center w-full truncate px-12">{loading ? '...' : store?.name || 'تفاصيل المتجر'}</h1>
                         {isOwner && (<div className="absolute left-2" ref={menuRef}><button onClick={() => setIsMenuOpen(p => !p)} className="p-2 rounded-full hover:bg-slate-700"><MoreIcon /></button>{isMenuOpen && (<div className="absolute left-0 mt-2 w-40 bg-slate-700 rounded-md shadow-lg z-10"><button onClick={handleDeleteStore} className="block w-full text-right px-4 py-2 text-sm text-red-400 hover:bg-slate-600">حذف المتجر</button></div>)}</div>)}
                    </div>
                </div>
            </header>
            <main className="container mx-auto px-4 py-6">
                 <div className="max-w-2xl mx-auto">
                     ...
                     {store && (
                        <div>
                            {storeImageUrl && ( <div className="aspect-video w-full bg-slate-700 rounded-lg overflow-hidden mb-6 border border-slate-700"><img src={storeImageUrl} alt={store.name} className="w-full h-full object-cover" /></div> )}
                                ...
                                <div className="flex justify-between items-start gap-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-400 text-sm mt-2">
