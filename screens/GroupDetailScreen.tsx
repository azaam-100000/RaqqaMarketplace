...
const GroupDetailScreen: React.FC = () => {
    ...
    const handleDeleteGroup = async () => {
        if (!group) return;
        if (window.confirm(`هل أنت متأكد من حذف مجموعة "${group.name}"؟ هذا الإجراء سيحذف جميع منشوراتها ولا يمكن التراجع عنه.`)) {
            try {
                if (group.cover_image_url) {
                    await supabase.storage.from('uploads').remove([group.cover_image_url]);
                }
    ...
    const renderContent = () => {
        ...
        if (group.is_private && !isMember && !isOwner) {
            return (
                 <div className="text-center text-slate-400 bg-slate-800 p-8 rounded-lg border border-slate-700">
                    <LockIcon />
                            ...
                            <div className="absolute left-2" ref={menuRef}>
                                ...
                                {isMenuOpen && (
                                    <div className="absolute left-0 mt-2 w-40 bg-slate-700 border border-slate-600 rounded-md shadow-lg z-10">
                                        <button onClick={handleDeleteGroup} className="block w-full text-right px-4 py-2 text-sm text-red-400 hover:bg-slate-600">حذف المجموعة</button>
                                    </div>
                                )}
                            </div>
                         )}
                     ...
                     {group && (
                        <div>
                            <div className="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden mb-6">
                                <div className="relative aspect-[16/7] w-full bg-slate-700">
                                    {coverImageUrl ? (
                                        <img src={coverImageUrl} alt={`${group.name} cover`} className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="w-full h-full bg-gradient-to-br from-slate-800 to-slate-700"></div>
                                    )}
                                </div>
                                <div className="p-4 space-y-4">
                                    ...
                                    {group.description && (
                                        <p className="text-slate-300 pt-4 border-t border-slate-700">{group.description}</p>
                                    )}
