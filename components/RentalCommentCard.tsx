...
interface RentalCommentCardProps {
  comment: RentalPostComment;
  onCommentUpdated: (updatedComment: RentalPostComment) => void;
  onCommentDeleted: (commentId: string) => void;
}

const RentalCommentCard: React.FC<RentalCommentCardProps> = ({ comment, onCommentUpdated, onCommentDeleted }) => {
    const { user } = useAuth();
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm('هل أنت متأكد من حذف هذا التعليق؟')) {
            const { error } = await supabase.from('rental_post_comments').delete().eq('id', comment.id);
            if (error) {
                alert(`فشل حذف التعليق: ${getErrorMessage(error)}`);
            } else {
    ...
    const handleUpdate = async () => {
        ...
        if (error) {
            ...
            const fullUpdatedComment: RentalPostComment = { ...comment, ...updatedComment };
    ...
    return (
        <div className="flex items-start gap-3">
            ...
            <div className="flex-1 bg-slate-700/50 rounded-lg px-4 py-2">
...
export default RentalCommentCard;
