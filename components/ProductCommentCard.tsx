...
const ProductCommentCard: React.FC<ProductCommentCardProps> = ({ comment, onCommentUpdated, onCommentDeleted }) => {
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm('هل أنت متأكد من حذف هذا التعليق؟')) {
            const { error } = await supabase.from('product_comments').delete().eq('id', comment.id);
            if (error) alert(`فشل حذف التعليق: ${getErrorMessage(error)}`);
    ...
    const handleUpdate = async () => {
        ...
        if (error) alert(`فشل تحديث التعليق: ${getErrorMessage(error)}`);
            ...
            // Re-attach existing profile data since it doesn't change on update.
            const fullUpdatedComment = {
                ...comment,
                ...updatedComment,
            };
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isOwner && !isEditing && (
                            <div className="relative" ref={menuRef}>
                                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-1 -mr-1 text-slate-400 hover:text-white hover:bg-slate-700 rounded-full"> <MoreIcon /> </button>
                                {isMenuOpen && (
